        ...
        if (item.isDropdown) {
                ...
                if (child.isButton) {
                    ...
                    childrenHTML += `
                        <a href="${child.href}" ${child.guard || ''}
                            style="display: flex; align-items: center; gap: 1rem; padding: 0 1rem; margin-top: 0.125rem; border-radius: 0.75rem; font-size: 0.75rem; transition: all 0.2s ease; ${isActive ? 'color: #2563eb; background: #eff6ff; font-weight: 700;' : 'color: #6b7280;'} text-decoration: none; width: 100%; min-height: 2.5rem;">
                            <svg style="width: 1rem; height: 1rem; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ${renderIcon(child.icon, child.iconPaths)}
                            </svg>
            ...
            navHTML += `
                <div id="${item.id}-parent" style="margin-top: 0.25rem; margin-bottom: 0.125rem; padding-left: 0.5rem; padding-right: 0.5rem;">
                    <button onclick="toggleSidebarDropdown('${item.id}')" style="width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0 1rem; border-radius: 0.75rem; font-size: 0.75rem; color: #4b5563; border: none; background: transparent; cursor: pointer; font-weight: 700; letter-spacing: 0.05em; min-height: 2.75rem;">
            ...
            navHTML += `
                ...
                <a href="${item.href}" ${item.guard || ''}
                    style="display: flex; align-items: center; gap: 1rem; padding: 0 1rem; border-radius: 0.75rem; font-size: 0.75rem; transition: all 0.2s ease; ${isActive ? 'color: #2563eb; background: rgba(59, 130, 246, 0.1); font-weight: 700;' : 'color: #6b7280;'} text-decoration: none; font-weight: 700; letter-spacing: 0.05em; width: 100%; min-height: 2.75rem;">
                    <svg style="width: 1.25rem; height: 1.25rem; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ${renderIcon(item.icon, item.iconPaths)}
                    </svg>
                    <span>${item.label}</span>
